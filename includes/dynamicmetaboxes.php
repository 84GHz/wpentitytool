...
class DynamicMetaBoxes
{
	...
	function mediabox($box, $post_id) {
        ...
        $html .= $box['title'] ;
        $html .= '</label>';
        $html .= '<input type="text" name="'. $box['id'] . '" value="'. $post_meta  .'" size="25"/>' ;
        $html .= '<button type="button" class="button" id="events_video_upload_btn" data-media-uploader-target="#' .$box['id'] . '" data-media-uploader-pic="'. $box['id'] .'-pic"> select media</button><br/><br/>';

        $html .= '<img src="' . $post_meta . '" id="'. $box['id'] .'-pic" style="max-width:250px;" />';
        echo $html;
